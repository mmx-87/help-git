...
- всю базу данных об истории разработки проекты
...
Когда мы доделалий какую-то часть работы, то мы фиксируем изменения из индекса в репозитории.
...
Если вы внесли изменения в репозиторий совместно, то откатывать изменения придётся тоже сразу на всё - и на то что работает, и на то что не работает
...
$ git add "filename"
```
добавляет файл "filename" в индекс, т.е. теперь файл "filename" проиндесксирован, и команда git status теперь покажет, что такой файл у нас есть и GIT следит за ним.

```
$ git add -p
```
позволяет индексировать не весь файл, а только некоторые изменения из него, которые мы может отдельно зафиксировать (внести в репозиторий).
...
$ git commit
```
теперь запустится [текстовый редактор](./config.md) с открытым файлом. В этом файле мы должны добавить сообщение о том, какие по сути мы вносим изменения в репозиторий. Первая строка записи изменений должна быть небольшой (обычно не более 50 символов) и довольно ёмко и при этом кратко описывать суть изменений.
...
также git commit сохраняет в правах на файл сведения о возможности выполнения этого файла для пользователя
...
$ git add .
```
добавляет в индекс все файлы и подкаталоги относительно текущего каталога.

Команда
```
$ git reset HEAD файл_или_каталог
```
сбрасывать/удаляет изменения в индексе
...
git status
```
покажет, что файл был переименован - т.к. файл добавили в индекс и GIT, проанализировав содержимое догадался, что было переименование
